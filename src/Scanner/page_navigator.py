...
class PageNavigator:
    ...
    def __goto_page(self, page_url: str):
    ...
    def __check_page_correctness(self, selector: str, error_message: str):
    ...
    def goto_login_page(self):
        try:
            self.__goto_page(self.login_url)
    ...
    def goto_best_matches_page(self):
        ...
        try:
            self.__goto_page(self.best_matches_url)
            self.__check_page_correctness('h2:has-text("Jobs you might like")', "It's not Best matches jobs page")
        except Exception as e:
            raise e

    def goto_profile_info_page(self):
        if not self.logged_in:
            return

        try:
            self.__goto_page(self.profile_info_url)
            #self.__check_page_correctness('h1:has-text("Contact info")', "It's not Contact info page")
        except Exception as e:
            raise e

    def goto_profile_extra_info_page(self):
        if not self.logged_in:
            return

        try:
            self.__goto_page(self.profile_id_url)
        except Exception as e:
            raise e
        else:
            ...
            try:
                self.__goto_page(self.profile_extra_info_url + cipher_id + '/details')
            except Exception as e:
                raise e
            else:
                soup = BeautifulSoup(self.page.content(), 'html.parser')
    ...
    def reenter_password(self):
        ...
        selector_element = self.page.query_selector('h1:has-text("Re-enter password")')
        if selector_element:
            self.logged_in = False
            self.page.locator('#sensitiveZone_password').fill(self.password)
    ...
    def reenter_answer(self):
        if not self.next_step_available:
            return

        selector_element = self.page.query_selector('h2:has-text("Confirm that it\'s you")')
        if selector_element:
            self.logged_in = False
